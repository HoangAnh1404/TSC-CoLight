...
from FL_PPO.TSCEnvironment.wrapper_utils import convert_state_to_static_information, find_index
...
class BaseTSCEnvWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self.incoming_edge_prefixes = set()
        self.episode = 0
    ...
    def reward_wrapper(self, states) -> float:
        """Only count waiting time of vehicles on incoming edges of this tls."""
        vehicle_state = states.get('vehicle', {}) or {}
        total_waiting_time = 0.0
        counted = 0
        for _, veh_info in vehicle_state.items():
            lane_id = veh_info.get('lane_id')
            if lane_id is None:
                continue
            edge_id = lane_id.split('_')[0]  # lane_id like edge_0
            if edge_id in self.incoming_edge_prefixes:
                total_waiting_time += veh_info.get('waiting_time', 0.0)
                counted += 1
        if counted == 0:
            # Fallback to global sum to avoid zero signal; keep consistent sign
            total_waiting_time = sum(v.get('waiting_time', 0.0) for v in vehicle_state.values())
        return -float(total_waiting_time)
    ...
    def reset(self, seed=1) -> Tuple[Any, Dict[str, Any]]:
        ...
        # Derive incoming edge prefixes from movement_ids (format like E2_l or E2--l)
        self.incoming_edge_prefixes = set()
        for mid in self.movement_ids:
            if '--' in mid:
                self.incoming_edge_prefixes.add(mid.split('--')[0])
            else:
                self.incoming_edge_prefixes.add(mid.split('_')[0])
